...
class LPGCN(nn.Module):
    ...
    def __init__(self, num_constraints, num_variables, num_layers=5):
        ...
        self.cv = nn.ModuleList([GCNConv(dims[l-1], 32, dims[l]) for l in range(1, self.num_layers)])
        self.cw = nn.ModuleList([GCNConv(dims[l-1], 32, dims[l]) for l in range(1, self.num_layers)])
